    ...
    >
      ...
      >
        ...
        {userPlants.length === 0 ? (
          <motion.div
            ...
            transition={{ duration: 0.3 }}
          >
            <p className="text-gray-600 mb-4">{t('dashboard.noPlants')}</p>
          ...
          <motion.div
            ...
            transition={{ duration: 0.3 }}
          >
            <p className="text-gray-600">{t('dashboard.allCaredFor')}</p>
          ...
          <div className="space-y-4">
            {plantsNeedingCare.map((plant) => {
              const actions = todaysActions.get(plant.id) || [];
              const localizedName = t(`plants.${plant.name}`, plant.name);
              return (
                <PlantActionCard
                  key={plant.id}
                  plant={{ ...plant, name: localizedName }}
                  actions={actions}
                />
              );
            })}
      ...
      >
        ...
        <div className="card">
          ...
          <p className="text-gray-600 mb-4">{getWeatherTip()}</p>
        </div>
      </motion.section>
    </motion.div>
  );
